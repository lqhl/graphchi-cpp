...
private:
    ...
    int niters;
#ifndef SPARSE_TABLE
    dense_hash_map < vid_t, std::vector<vid_t> > walks[2];
    std::vector <dense_hash_map <vid_t, uint16_t> > counter;
#else
    sparse_hash_map < vid_t, std::vector<vid_t> > walks[2];
    std::vector <sparse_hash_map <vid_t, uint16_t> > counter;
#endif
    std::vector <int> total;

public:
    PageRankProgram(metrics &_m, vid_t source_start, vid_t source_end, int R, int niters):
            m(_m), source_start(source_start), source_end(source_end),
            source_len(source_end - source_start + 1), cur_walks(0), R(R),
            niters(niters), counter(source_len), total(source_len) {
        assert(source_start <= source_end);

#ifndef SPARSE_TABLE
        walks[0].set_empty_key(-1);
        walks[1].set_empty_key(-1);
        ...
        for (vid_t source = source_start; source <= source_end; source++)
            for (int j = 0; j < R; j++) {
                std::vector<vid_t> &v = walks[cur_walks][source];
                v.push_back(source);
            }
    }
    ...
    void update(graphchi_vertex<VertexDataType, EdgeDataType> &v, graphchi_context &gcontext) {
        if (walks[cur_walks].find(v.id()) != walks[cur_walks].end()) {
            std::vector<vid_t> &local_walks = walks[cur_walks][v.id()];
            for (std::vector<vid_t>::iterator it = local_walks.begin(); it != local_walks.end(); it++) {
                vid_t source = *it;
                counter[source-source_start][v.id()]++;
                total[source-source_start]++;
                if ((float)rand() / (float)RAND_MAX <= 0.15) {
                    /* walks[1-cur_walks][source].push_back(1); */
                } else if (v.num_inedges() > 0) {
                    vid_t next = v.inedge(rand() % v.num_inedges())->vertex_id();
                    walks[1-cur_walks][next].push_back(source);
                    if (scheduler)
                        gcontext.scheduler->add_task(next);
                } else {
                    walks[1-cur_walks][source].push_back(source);
                    if (scheduler)
    ...
    void before_iteration(int iteration, graphchi_context &ginfo) {
        printf("# of scheduled nodes: %zu\n", walks[cur_walks].size());
    }

    /**
     * Called after an iteration has finished. Not implemented.  */
    void after_iteration(int iteration, graphchi_context &ginfo) {
        walks[cur_walks].clear();
...
int main(int argc, char ** argv) {
    ...
    int exec_threads     = get_option_int("execthreads", 0);
    ...
    if (source_range % num_tasks == 0)
        sources_per_node = source_range / num_tasks;
    else
        sources_per_node = source_range / num_tasks + 1;
    ...
    PageRankProgram program(m, cur_start, cur_end, R, niters);
    engine.run(program, niters);

    if (print)
        for (vid_t source = cur_start; source <= std::min(cur_end, 10u); source++) {
            std::vector<std::pair<vid_t, float> > ret = program.result(source);
